...
class DisIsAllYouNeed(Feature):
    def create_features(self):
        ...
        test_df_list = []
        for typ in types:
            train_tmp = build_couple_dataframe(train_csv, structures_csv, typ, n_atoms=10)
            test_tmp = build_couple_dataframe(test_csv, structures_csv, typ, n_atoms=10)
            train_df_list.append(train_tmp)
            test_df_list.append(test_tmp)

        train_df = pd.concat(train_df_list, axis=0)
        test_df = pd.concat(test_df_list, axis=0)
        cols = [col for col in train_df.columns if "atom_" in col or "d_" in col]
        cols.append("molecule_index")
        train_df = train_df[cols]
        test_df = test_df[cols]

        train_csv = train_csv.merge(train_df, on=["molecule_index", "atom_index_0", "atom_index_1"], how="left")
        test_csv = test_csv.merge(test_df, on=["molecule_index", "atom_index_0", "atom_index_1"], how="left")

        new_cols = ['atom_2', 'atom_3', 'atom_4', 'atom_5', 'atom_6', 'atom_7', 'atom_8',
        ...
        for col in new_cols:
            self.train[col] = train_csv[col]
            self.test[col] = test_csv[col]     
